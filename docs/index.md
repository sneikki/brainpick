...
<!-- brainpick:begin index (hash:dde034de) -->
...
- [ADR: the brain template lives in brainpick](reference/adr/brain-template-in-brainpick.md) — Why the brain starter template moved from henxels into brainpick (brainpick init --template brain): what the template teaches depends on how brainpick writes, and a template in another repository drifted from it; henxels stays the referee of the contract the template writes.
...
- [brainpick init](reference/cli/init.md) — Detect the bundle and backends, write config, and compile T1 — the one command from zero to a living brain; --template brain scaffolds the brain first.
